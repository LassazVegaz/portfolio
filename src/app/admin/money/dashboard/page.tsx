...
import DashboardFilters from "../transactions/components/DashboardFilters";
import { TransactionRow } from "../transactions/components/Mobile";
import Stat from "../transactions/components/Stat";
import MoneyCharts from "../transactions/MoneyCharts";
...
export default async function DashboardPage({
  ...
  const pageHref = (targetPage: number) => {
    ...
    return `/admin/money/dashboard?${params}` as Route;
  };

  return (
    <main className="admin-shell min-h-screen pb-16">
      <PageContainer className="mx-auto min-w-[1000px] max-w-7xl">
        <TopNavigator links={["home", "money"]} />
        <div className="mt-8 flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="admin-eyebrow">SGD ledger</p>
            <h1 className="mt-2 text-3xl font-semibold">Money dashboard</h1>
          </div>
          <Link
            href="/admin/money/transactions"
            className="admin-secondary-button inline-flex"
          >
            View transactions
          </Link>
        </div>

        <div className="mt-page block">
          <DashboardFilters
        ...
        >
          ...
          <div className="grid grid-cols-[1.4fr_.6fr_.9fr_.8fr_.6fr_.8fr] gap-4 border-y border-admin-line px-5 py-3 text-xs uppercase tracking-wider text-admin-muted md:grid">
